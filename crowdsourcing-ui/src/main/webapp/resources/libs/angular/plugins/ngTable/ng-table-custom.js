...
app.factory('ngTableParams', ['$q', '$log', function ($q, $log) {
    ...
    var ngTableParams = function (baseParameters, baseSettings) {
        ...
        /**
         * @ngdoc method
         * @name ngTable.factory:ngTableParams#pageCount
         * @methodOf ngTable.factory:ngTableParams
...
angular.module('ngTable').run(['$templateCache', function ($templateCache) {
	...
	$templateCache.put('ng-table/pager.html', '<div class="ng-cloak responsivePager"><div ng-if="params.settings().counts.length" class="ng-table-counts btn-group pull-right pagingSizeControl"><span for="pagingDropDown">Display: </span><select id="pagingDropdown" ng-model="params.parameters().count" ng-options="count for count in params.settings().counts" ng-change="params.count(params.parameters().count)" ></select></div><ul class="pagination"> <li ng-class="{\'disabled\': !page.active}" ng-repeat="page in pages" ng-switch="page.type"><a ng-switch-when="first" ng-click="params.page(page.number)" href=""><div class="pagingFirstPage"><div class="pagingFirstBar"></div></div></a><a ng-switch-when="prev" ng-click="params.page(page.number)" href=""><div class="pagingFirstPage pagingPrevPage"></div></a> <a ng-switch-when="goto" href="" class="pagingGoto">Page: <input type="number" min="1" max="{{params.pageCount()}}" ng-model="params.parameters().page" validate/> of {{params.pageCount()}}</a> <a ng-switch-when="next" ng-click="params.page(page.number)" href="" class="pagingNext"><div class="pagingLastPage pagingNextPage"></div></a><a ng-switch-when="last" ng-click="params.page(page.number)" href="" class="pagingLast"><div class="pagingLastPage"><div class="pagingLastBar"></div></div></a></li></ul><div class="pagingRecordStats">{{params.page() === 1 ? 1 : (((params.page() - 1)  * params.count())+1)}} - {{ (params.page() * params.count() > params.total()) ? params.total() : params.page() * params.count()  }} of {{params.total()}}</div></div>');
}]);
    return app;
}));